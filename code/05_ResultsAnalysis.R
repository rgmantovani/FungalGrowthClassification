...
cat(" @ Loading results \n")
if(!file.exists("~/capsule/results/mlr_results_complete.RData")) {
 	stop(" - There is no results' file! Please, run ML experiments first.\n")
 	q(save = "no",  runLast = FALSE)
} else {
	load(file = "~/capsule/results/mlr_results_complete.RData", verbose = TRUE)	
}
...
# gf
ggsave(gf, file = "~/capsule/plots/fig_algorithms_performance.pdf", width = 6.29, height = 7.54)
...
write.csv(df.stats, file = "~/capsule/results/stats_original_vs_balanced.csv")
...
# Now we can save it
ggsave(g2, file = "~/capsule/plots/fig_original_vs_balanced.pdf", width = 7.55, height = 5.47)
...
ggsave(g3, file = "~/capsule/plots/fig_average_ranking.pdf", width = 7.99, height = 4.39)
...
# g5 
ggsave(g5, file = "~/capsule/plots/fig_voting_predictions.pdf", width = 7.45, height = 5.81)
...
df_all2_resc = read.csv(file = "~/capsule/data/dataset/task_aggregated.csv", row.names = 1)
...
options(OutDec = ",")     
png("~/capsule/plots/fig_decision_tree.png", units="in", width=12.8, height=7.2, res=300, pointsize = 20)
...
ggsave(g_importance, file = "~/capsule/plots/fig_randomForest.pdf", units = "in", width = 9, 
...
all.files = list.files(path = "~/capsule/results/deepLearningResults", full.names = TRUE)
...
aux.dl = lapply(dl.files,function(file) {
	# print(file) # debug
	algo.name = gsub(x = file, pattern = paste0("~/capsule/results/deepLearningResults/|.csv"), 
		replacement = "")
...
# g6 
ggsave(g6, file = "~/capsule/plots/fig_dl_vs_traditionalML.pdf", width = 5.73, height = 4.04)
